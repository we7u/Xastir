...
#else
    if (DLM_queue_len()>0) DLM_do_transfers();
#endif
...
static void DLM_queue_progress(int prog) {
    if (prog) {
        ...
        if (DLM_progress_callback!=NULL) DLM_progress_callback();
    }
}
...
static void DLM_queue_entry_free(struct DLM_queue_entry *q) {
    if (q) {
        ...
        if ((q->state==DLM_Q_IDLE) || (q->state==DLM_Q_STOP)) {
            ...
            if (DLM_queue == q) DLM_queue = q->next;
...
#ifdef HAVE_LIBCURL
#ifdef USE_CURL_MULTI
            if (q->curlErrBuf)  free(q->curlErrBuf);
            if (q->curlSession) curl_easy_cleanup(q->curlSession);
            if (q->stream)      fclose(q->stream);
#endif
#endif
...
static struct DLM_queue_entry *DLM_get_next_tile(int state) {
    ...
    if (q) begin_critical_section(&(q->lock), "DLM_transfer_thread: tile lock");
...
#ifdef DLM_QUEUE_THREADED
        if (runningTransfers>0) idleCnt=0;
#endif
        ...
        if ((USE_CURL_MULTI < 1) || (runningTransfers < USE_CURL_MULTI)) {
            ...
            if (tile) {
...
#ifdef HAVE_LIBCURL
#ifdef USE_CURL_MULTI
                if (!tile->curlErrBuf)  tile->curlErrBuf=malloc(CURL_ERROR_SIZE);
                if (!tile->curlSession) tile->curlSession=DLM_curl_init(tile->curlErrBuf);
                ...
                if (curl_result != CURLE_OK) {
                    ...
                    if (!DLM_store_file(tile)) DLM_queue_progress(1);
                }
